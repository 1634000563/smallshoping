# Task 027 — AI Slice：“老张上次那些螺丝再来两盒”
...
### Vertical Slice 强制要求
完整链路必须是：Input → Intent → Entity/Context → Tool → Risk → Domain → Ledger → DB → Response。
必须同时保留一个不依赖 AI 的人工兜底入口，并验证两条路径最终得到相同业务事实。
### 记忆/上下文约束
结构化账务事实以数据库为准。长期记忆只能保存经过验证的偏好、别名、规则和引用，不能替代 Ledger，也不能把全库复制进 Prompt。
