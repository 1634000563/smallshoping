...
## ADR-018 进货时商品不存在 → 顺手建档再入库（Task 059 真机验收）

决定：purchase_in 遇到不存在的商品时不再反问「先建一个？」，直接以数量单位建档（售价 0 元待改价，有进价则记成本）并继续入库，回复明确告知「刚建了商品「X」」。
原因：老板说「进100斤土豆」本身就是最明确的意图——没有这个商品就建档再入库是唯一合理解释，反问是多此一举的交互摩擦（真机验收反馈）。「不确定的实体不能猜」宪法不适用于此：这里不是猜测老板指哪个商品，而是执行采购指令的必要建档步骤。
影响：销售（add_sale_item）遇到不存在的商品仍然反问（销售场景无成本信息，自动建档风险高）；新建商品价格一律 0 元，由老板说「X改成X块」定价（不猜测价格）。
